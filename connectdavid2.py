    ...
    while True:
        try:        
            ...
            if user_input.lower().startswith("p:"):
                full_history += f"[PATTERN_EXPLANATION]:\n{user_input}\n\n"
                p_last_text = "David, memorize this pattern which is derived from Wei's experiences and explain briefly if you understand it."
                p_mode_text = "David is memorizing the pattern...\n"
                p_response_text = "[DAVID_ACKNOWLEDGEMENT]:\n"
                p_content = ""
                ...
                p_knowledge_content = get_matched_knowledge(text_vectorstore, user_input, 10)
                if p_knowledge_content.strip():
                    print("\nKnowledge contents was found.")      # print(f"\n[EXPERIENCE_FOUND]:\n{p_knowledge_content}\n")
                    p_content += f"[THEORY_AND_KNOWLEDGE_FROM_MEMORY]:\n{p_knowledge_content}\n\n"
                else:
                ...
                if found_images:
                    ...
                    if matched_ohlc:
                        full_history += system_note + matched_ohlc[0]
                else:
                    print("No pattern to be memorized!\n")
                    continue

                p_content += f"Matched patterns for [IMAGE 1]: {found_images[0]}\n\n---\n\n"
                p_content += find_matched_pattern(image_vectorstore, img, k=100, distance_threshold=0.7)
                p_content += "\n\n" + "=" * 30 + "\n\n"

                p_content += full_history + p_last_text
                p_prompt = [
                    {'role': 'system', 'content': SYSTEM_PROMPT},
                    {'role': 'user', 'content': p_content, 'images': found_images}
                ]

                # ประเมินปริมาณ Tokens ทั้งหมดที่ต้องใช้ในการส่งให้ข้อความและภาพให้ David ผ่าน prompt
                p_text_tokens, p_image_tokens = get_token_count(p_prompt)
                print(f"[TOKEN ESTIMATION]: Text input = {p_text_tokens} | Image input = {p_image_tokens} | Total input = {p_text_tokens + p_image_tokens}\n")

                p_analysis = david_chat(p_prompt, p_mode_text, p_response_rtext)
                # ถ้าเกิด ollama.chat เกิด Crash หรือทำงานไม่สำเร็จ หรือ ollama ตัดการทำงานของ David ให้กลับไปลูปถาม Input ใหม่
                if not p_analysis.strip():
                    print("\nError: Empty Response\n")
                else:
                    try:
                        category_to_save = input("Name of pattern ('none' for not save): ").lower().strip()
                        if to_save == "none":
                            break
                        else:
                            save_image_to_brain(image_vectorstore, category_to_save, full_history, found_images[0])
                            break
            ...
            # ตรวจสอบว่าเป็นเป็นการให้ KNOWLEDGE หรือไม่
                ...
                k_last_text = "David, acknowledge this knowledge which is derived from Wei's experiences and explain briefly if you understand it."
                k_mode_text = "David is absorbing your knowledge...\n"
                k_response_text = "[DAVID_ACKNOWLEDGEMENT]:\n"
            ...
            # ตรวจสอบดูว่าเป็นชุดคำถามหรือไม่
                ...
                q_mode_text = "David is thinking...\n"
                q_response_text = "[DAVID_ANALYSIS]:\n"
                q_content = ""

                # ค้นหา THEORY และ KNOWLEDGE ที่เกี่ยวข้องกับ user_input
                q_knowledge_content = get_matched_knowledge(text_vectorstore, user_input, 10)
                if q_knowledge_content.strip():
                    print("\nKnowledge contents was found.")      # print(f"\n[EXPERIENCE_FOUND]:\n{q_knowledge_content}\n")
                    q_content += f"[THEORY_AND_KNOWLEDGE_FROM_MEMORY]:\n{q_knowledge_content}\n\n"
                else:
                    print("\nKnowledge contents could not be found.")
                ...
                if found_images:
                    ...
                    if matched_ohlc:
                        for str_ohlc in matched_ohlc:
                            full_history += system_note + str_ohlc

                for i, img in enumerate(found_images, start=1):
                    q_content += f"Matched patterns for [IMAGE {i}]: {img}\n\n---\n\n"
                    q_content += find_matched_pattern(image_vectorstore, img, k=10, distance_threshold=0.5)
                    q_content += "\n\n" + "=" * 30 + "\n\n"

                q_content += full_history
                q_prompt = [
                    {'role': 'system', 'content': SYSTEM_PROMPT},
                    {'role': 'user', 'content': q_content, 'images': found_images}
                ]

                # ประเมินปริมาณ Tokens ทั้งหมดที่ต้องใช้ในการส่งให้ข้อความและภาพให้ David ผ่าน prompt
                q_text_tokens, q_image_tokens = get_token_count(q_prompt)
                print(f"[TOKEN ESTIMATION]: Text input = {q_text_tokens} | Image input = {q_image_tokens} | Total input = {q_text_tokens + q_image_tokens}\n")

                q_analysis = david_chat(q_prompt, q_mode_text, q_response_rtext)
                # ถ้าเกิด ollama.chat เกิด Crash หรือทำงานไม่สำเร็จ หรือ ollama ตัดการทำงานของ David ให้กลับไปลูปถาม Input ใหม่
                if not q_analysis.strip():
                    print("\nError: Empty Response\n")
                    continue
                else:
                ...
                try:
                    ...
                    if feedback == "y":
                        ...
                        if is_Pattern == False:
                            ...
                            save_text_to_brain(text_vectorstore, full_history, current_time_string)
                        else:
                            save_image_to_brain(image_vectorstore, full_history, total_images_list[0])
                        ...
                        next_found_images, next_system_note = find_images_in_content(correction_input)
                        if next_found_images:
                            ...
                            for j, _img in enumerate(next_found_images, start=len(total_images_list)+1):
                                print(f"\n--- Configuring Data for IMAGE {j} ---")
        
                                # เรียกฟังก์ชันตั้งค่า Chart สำหรับภาพนี้
                                _chart_configuration = config_chart_and_bars(_img)
                                # ตรวจสอบว่ามีการ input ค่าต่างๆก่อนที่จะ Unpack
                                if _chart_configuration is not None:
                                    _symbol, _exchange, _timeframe, _start_time, _end_time = _chart_configuration
                    
                                _ohlc_path = os.path.join(OHLC_PATH, f"{_symbol}_{_exchange}_{_timeframe}.csv")
                                _df_filtered = fetch_OHLC(_ohlc_path, _start_time, _end_time)
                    
                                if _df_filtered is not None and not _df_filtered.empty:
                                    # ใช้ index=True หากเวลาของคุณอยู่ที่ Index
                                    _ohlc_reference = _df_filtered[['open', 'high', 'low', 'close']].to_string(index=True)

                                    # รับค่าตำแหน่ง Bar และเวลาที่สัมพันธ์กับภาพนี้
                                    _index_bar_no = input(f"[IMAGE{j}: {_img}] Choosing index bar no: ")
                                    _time_related_bar = input(f"Time related to index bar (YYYY-MM-DD HH:MM:SS) for Image {j}: ")
                                
                                    # สะสมข้อมูลเข้าใน ohlc_content โดยระบุว่าอ้างอิงกับ Image ลำดับที่เท่าไหร่
                                    _ohlc_content += f"\n[DATA FOR IMAGE {i}]:\n"
                                    _ohlc_content += f"Bar {_index_bar_no} is at {_time_related_bar}. Data reference:\n\n{_ohlc_reference}\n"
                                    _ohlc_content += "-" * 30 + "\n"
                                    full_history += _ohlc_content
                                else:
                                    print(f"Warning: No OHLC data found for IMAGE {j}")
                            else:
                                print(f"Skipping configuration for IMAGE {j}")
                        else:
...
# ฟังชั่นใช้ image_embed ที่เป็น CLIP ค้นหาภาพที่ระบุให้ค้นหาตาม query
def get_matched_image(vectorstore, query, k=10):
    if not vectorstore:
        return []
    
    # ใช้ image_embed เปลี่ยน 'ข้อความคำถาม' ให้เป็น 'พิกัดภาพ'
    query_vector = image_embed.encode([query])[0].tolist()
    results = vectorstore.similarity_search_with_score_by_vector(query_vector, k=k)
    
    return [res[0].metadata['path'] for res in results]
...
def fetch_OHLC(image_list, start=1):
    ...
    for i, img in enumerate(image_list, start=start):
        ...
        tmp_time1 = input("Start time (HH:MM:SS): ").strip()
        tmp_time2 = input("End time (HH:MM:SS): ").strip()
    
        start_time = f"{date} {tmp_time1}"
        end_time = f"{date} {tmp_time2}"
...
def save_image_to_brain(vectorstore, category, description, image_file):
    ...
    doc = Document(page_content=des, metadata={"category": category, "source": image_file, "path": file_path})
...
def create_brain():    
    ...
    # --- ส่วนที่ 3: รับประสบการณ์ (PATTERN) จากบันทึกภาพ image_journal ---
    if os.path.exists(CHART_PATH) and os.path.exists(LESSON_PATH) and text_vectorstore:
        ...
        print(f"Absorbing pattern from charts... [{len(pairs)} unit(s)]")
        
        for unit in pairs:
            chart_path = os.path.join(CHART_PATH, chart_file[unit])
            with open(os.path.join(CHART_PATH, f"{unit}.txt"), "r", encoding="utf-8") as f:
                content_text = f.read()
...
# --- ฟังก์ชันช่วยเหลือสำหรับการส่งภาพ ---
