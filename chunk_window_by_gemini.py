...
# --- GUI Reasoning Window Class ---
class ReasoningWindow:
    def __init__(self, title="David's Internal Monologue"):
        self.root = tk.Tk()
        self.root.title(title)
        self.root.geometry("600x400")
        self.root.attributes("-topmost", True)  # ให้อยู่ด้านบนเสมอเพื่อให้เห็นง่าย
...
def main():
    ...
    while True:
        try:        
            ...
            # --- เริ่มต้นระบบ Pop-up Window ---
            reasoning_gui = ReasoningWindow(title="David's Initial Analysis")
            gui_thread = threading.Thread(target=reasoning_gui.start, daemon=True)
            gui_thread.start()
            ...
            for chunk in stream:
                if 'message' in chunk and 'content' in chunk['message']:
                    ...
                    # ส่ง content เข้า Pop-up
                    if reasoning_gui.is_running:
                        reasoning_gui.send_message(content)
                    
                    # เก็บข้อมูลเข้าตัวแปรหลักเพื่อใช้ในระบบ Memory
                    if is_first_chunk:
            ...
            # เมื่อ Stream จบ พิมพ์ผลลัพธ์สุดท้ายลง CMD
            ...
            while True:
                try:
                    ...
                    if feedback == "y":
                        ...
                        # --- Pop-up สำหรับช่วง Refinement ---
                        refine_gui = ReasoningWindow(title="David's Refinement Thought")
                        refine_gui_thread = threading.Thread(target=refine_gui.start, daemon=True)
                        refine_gui_thread.start()
                        ...
                        for chunk in refined_stream:
                            if 'message' in chunk and 'content' in chunk['message']:
                                content = chunk['message']['content']
                                if refine_gui.is_running:
                                    refine_gui.send_message(content)
