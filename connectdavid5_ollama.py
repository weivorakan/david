...
# บังคับใช้ SDPA ซึ่งให้ความเร็วใกล้เคียง Flash Attention โดยไม่ต้องติดตั้งเพิ่ม
...
Before giving the final answer(output), please provide a brief internal monologue or step-by-step reasoning explaining how you interpreted the request and which parts of the retrieved context you are using to formulate your response.
...
options_config = {
    'num_ctx': 12288,             # ขนาดสมองรวม Total Tokens/Input (จำได้ยาวขึ้น)
    'num_predict': 4096,          # เพดานคำพูด Output Tokens (ถ้าไม่เกินก็ไม่ให้โดนตัดบทดื้อๆ)
    'num_thread': 8,              # บังคับใช้ CPU Cores ตามจำนวน 6 Performance Cores (12 threads) และ 8 Efficiency Cores (8 threads) ของ Intel(R) Core(TM) i5-14600K (3.50 GHz)
#    'num_gpu': 26,                # ใช้ GPU แค่ 26 layers จากทั้งหมด 30 layers สำหรับ Gemma4-26B
...
def main():
    ...
    while True:
        try:        
                ...
                if os.path.exists(file_path):
                    with open(file_path, "r", encoding="utf-8") as f:
                        ...
                        if not user_input:
                            ...
                            tmp_name = os.path.splitext(file_name)[0]
                            name_parts = tmp_nmae.split('_')
                            pattern_name = " ".join(name_parts[:-1])
            ...
            if found_images:
                ...
                if is_pattern == True:
                    if image_vectorstore != None:
                        ...
                        matched_patterns = find_matched_pattern(image_vectorstore, found_images[0], k=3, distance_threshold=0.3)
                        if matched_patterns != None:
                            print(f"Found {len(matched_patterns)} matched pattern(s).")
                            prompt_content += f"Matched patterns for [IMAGE 1]: {found_images[0]}\n\n---\n\n"
                            prompt_content += matched_patterns + "\n\n---\n\n"
                else:
                    if image_vectorstore != None:
                        ...
                        matched_patterns = find_matched_pattern(image_vectorstore, found_images[0], k=10, distance_threshold=0.5)
                        if matched_patterns != None:
                            print(f"Found {len(matched_patterns)} matched pattern(s).")
                            prompt_content += f"Matched patterns for [IMAGE 1]: {found_images[0]}\n\n---\n\n"
                            prompt_content += matched_patterns + "\n\n---\n\n"
            ...
            text_tokens, image_tokens = token_count(initial_prompt)
                ...
                try:
                    ...
                    if feedback == "y":
                        ...
                        if r_found_images:
                            ...
                            #             r_matched_patterns = find_matched_pattern(image_vectorstore, img, k=3, distance_threshold=0.3)
                            #             if r_matched_patterns != None
                            #                 refined_prompt_content += f"Matched patterns for [IMAGE {i}]: {img}\n\n---\n\n"
                            #                 refined_prompt_content += r_matched_patterns + "\n\n---\n\n"
                            # else:
                            #     if image_vectorstore != None:
                            #         for i, img in enumerate(r_found_images, start=len(total_images_list)+1):
                            #             r_matched_patterns = find_matched_pattern(image_vectorstore, img, k=10, distance_threshold=0.5)
                            #             if r_matched_patterns != None
                            #                 refined_prompt_content += f"Matched patterns for [IMAGE {i}]: {img}\n\n---\n\n"
                            #                 refined_prompt_content += r_matched_patterns + "\n\n---\n\n"
                        else:
                        ...
                        r_text_tokens, r_image_tokens = token_count(refined_prompt)
...
def save_image_to_brain(vectorstore, category, summary_content, full_content, file_path):
    ...
    if category.upper() != "KNOWLEDGE-EXPERIENCE":
...
def save_text_to_brain(vectorstore, summary_content, full_content, file_path):
    ...
    txt_metadata = {"category": "KNOWLEDGE-EXPERIENCE", "source": txt_name}
    # 3. สร้าง Document ชั่วคราวขึ้นมาด้วย Tag 'THEORY-KNOWLEDGE-EXPERIENCE' และหั่นออกมาเป็นชิ้นเล็กๆ (Chunks)
...
def fetch_OHLC(image_list, start=1):
    ...
    for i, img in enumerate(image_list, start=start):
        ...
        start_time = f"{start_date} {tmp_start_time}"
        end_time = f"{end_date} {tmp_end_time}"
        ...
        ohlc_content = f"\n[OHLC_Index for IMAGE {i}] (Format: Datetime, O, H, L, C)\n"
...
def get_matched_knowledge(vectorstore, query, k=10):
    ...
    for doc in results:
        ...
        category = doc.metadata.get('category', 'KNOWLEDGE-EXPERIENCE').upper()
...
def david_chat(prompt, mode_text, response_text):
    ...
    if chunk:
        ...
        # --ทำการ update ข้อมูล Tokens ที่ใช้จริงทั้งหมดลงไปในไฟล์ total_tokens.txt โดยข้อมูลแต่ละบรรทัดจะแยกเป็นแต่ละเดือนๆไป
        ...
        if books:
            ...
            for i, file in enumerate(books):
                ...
                if loader:
                    ...
                    for d in data:
                        d.metadata["category"] = "THEORY-KNOWLEDGE"
