...
MODEL_NAME = "gemma-4-26B-A4B-it-GGUF:UD-IQ3_S"
...
def main():
    ...
    while True:
        try:        
            ...
            print("** Type 'file' for loading a text file **")
            ...
            if is_lesson == True:
                ...
                if experience_content.strip():
                    print("\nExperience contents was found.")      # print(f"\n[EXPERIENCE_FOUND]:\n{experience_content}\n")
                else:
            ...
            while True:
                try:
                    ...
                    if feedback == "y":
                        ...
                        save_text_to_brain(vectorstore, full_history, current_time_string)
...
def save_text_to_brain(vectorstore, content, text_file):
    ...
    temp_doc = Document(page_content=content, metadata={"category": "EXPERIENCE", "source": text_file})
...
def save_image_to_brain(vectorstore, content, image_file):
    ...
    doc = Document(page_content=content, metadata={"category": "EXPERIENCE", "source": image_file, "path": file_path})
...
def find_matched_charts(vectorstore, query, k=10):
    if not vectorstore:
        return []
...
def create_brain():    
    ...
    if os.path.exists(JOURNAL_PATH) and text_vectorstore:
        ...
        if journals:
            print(f"Absorbing Experiences from journals... [{len(journals)} unit(s)]")
            journal_docs = []
            
            for file in text_journals:
    ...
    # --- ส่วนที่ 3: รับประสบการณ์ (EXPERIENCES) จากบันทึกภาพ image_journal ---
    if os.path.exists(CHART_PATH) and os.path.exists(LESSON_PATH) and text_vectorstore:
        ...
        print(f"Absorbing Experiences from charts... [{len(pairs)} unit(s)]")
        ...
        for unit in pairs:
            ...
            doc = Document(
                ...
                metadata={"category": "EXPERIENCE", "source": unit, "path": chart_path}
            )
