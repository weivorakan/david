...
# --- GUI Reasoning Window Class (Fixed: Non-threaded, uses root.update()) ---
class ReasoningWindow:
    """
    แก้ไข: ไม่ใช้ threading แล้ว เพราะ tkinter ต้องรันบน Main Thread เท่านั้น
    วิธีใหม่: สร้างหน้าต่างบน Main Thread แล้วเรียก self.update()
    ภายใน streaming loop เพื่อให้ UI refresh ได้ทุก chunk
    """
    def __init__(self, title="David's Internal Monologue"):
        self.title = title
    ...
    def open(self):
        """เปิดหน้าต่างบน Main Thread — เรียกก่อนเริ่ม streaming loop"""
        self.root = tk.Tk()
        ...
        self.root.update()  # แสดงหน้าต่างทันที

    def append_text(self, msg):
        """เพิ่มข้อความลง text area — เรียกได้โดยตรงจาก Main Thread"""
    ...
    def update(self):
        """
        Refresh UI — เรียกภายใน for chunk in stream: loop ทุก iteration
        เพื่อให้หน้าต่างตอบสนองและแสดงข้อความได้ระหว่าง streaming
        """
    ...
    def close(self):
        """ปิดหน้าต่าง — เรียกหลัง streaming loop จบ (ถ้าต้องการปิดอัตโนมัติ)"""
        self.is_running = False
        if self.root:
            try:
                self.root.destroy()
    ...
    def on_closing(self):
        """จัดการกรณีผู้ใช้กดปิดหน้าต่างเอง"""
        self.is_running = False
        if self.root:
            self.root.destroy()
            self.root = None
...
def main():
    ...
    while True:
        try:        
            ...
            # --- แก้ไข: สร้างหน้าต่างบน Main Thread แล้วเรียก open() ทันที ---
            reasoning_gui = ReasoningWindow(title="David's Initial Analysis")
            reasoning_gui.open()  # หน้าต่างแสดงขึ้นมาทันที
            ...
            for chunk in stream:
                if 'message' in chunk and 'content' in chunk['message']:
                    ...
                    # เพิ่มข้อความและ refresh UI ทุก chunk — แทนที่ send_message() แบบเดิม
                    reasoning_gui.append_text(content)
                    reasoning_gui.update()  # <-- บรรทัดสำคัญ: ทำให้หน้าต่าง update ทุก chunk
            ...
            reasoning_gui.update()  # refresh ครั้งสุดท้ายหลัง loop จบ
            ...
            while True:
                try:
                    ...
                    if feedback == "y":
                        ...
                        # --- แก้ไข: หน้าต่าง Refinement ใช้วิธีเดิมกับ initial ---
                        refine_gui = ReasoningWindow(title="David's Refinement Thought")
                        refine_gui.open()  # หน้าต่างแสดงขึ้นมาทันที
                        ...
                        for chunk in refined_stream:
                            if 'message' in chunk and 'content' in chunk['message']:
                                ...
                                # เพิ่มข้อความและ refresh UI ทุก chunk
                                refine_gui.append_text(content)
                                refine_gui.update()  # <-- บรรทัดสำคัญ
                        ...
                        print("\n" + "-"*30)
                        refine_gui.update()  # refresh ครั้งสุดท้ายหลัง loop จบ
...
# --- ฟังก์ชันเสริม (save_to_brain, get_content_images, ฯลฯ) คงเดิมตามไฟล์ connectdavid2.py ของคุณ ---
