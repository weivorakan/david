...
model_name = "unsloth_gemma-4-26B-A4B-it-UD-IQ4_NL_XL.gguf"
clip_model_name = "prithivMLmods_gemma-4-26B-A4B-it.mmproj-q8_0.gguf"
BASE_DIR = r"C:\david"
MODEL_PATH = os.path.join(BASE_DIR, "ai_models", model_name)
CLIP_MODEL_PATH = os.path.join(BASE_DIR, "ai_models", clip_model_name)

# 2. ตัวแปรเก็บ Instance ของโมเดล (ยังไม่ได้โหลด)
ai_model = None 
...
async def lifespan(app: FastAPI):
    ...
    try:
        ai_model = Llama(
            ...
            n_ctx=12288,          # กำหนดขนาด Context Window (Total Tokens/Input)
            ...
            # type_k=4,           # บีบอัด KV Cache เป็น Q4_0
            # type_v=4,
            flash_attn=True,
            verbose=True
        )
...
async def analyze_chart(request: AnalysisRequest):
    ...
    def stream_generator():
        stream = ai_model.create_chat_completion(
            ...
            stream=True,        # เปิดโหมดส่งทีละก้อน
